...
Graphic_engine *graphic_engine_create()
{
  ...
  ge->banner = screen_area_init(1, HEIGHT_MAP + 2, WIDTH_MAP + WIDTH_DES + 1, HEIGHT_BAN);
  ge->help = screen_area_init(1, HEIGHT_MAP + HEIGHT_BAN + 3, WIDTH_MAP + WIDTH_DES + 1, HEIGHT_HLP);
  ge->feedback = screen_area_init(1, HEIGHT_MAP + HEIGHT_BAN + HEIGHT_HLP + 4, WIDTH_MAP + WIDTH_DES + 1, HEIGHT_FDB);
...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game)
{
  ...
  if (p_loc != NO_ID)
  {
    ...
    for (i = 0; i < 5; i++)
    {
      ...
      if (i == 2)
      {
        if (game_get_connection(game, space_get_id(space_act), W) != NO_ID)
          strcpy(w_link, "<-");
        if (game_get_connection(game, space_get_id(space_act), E) != NO_ID)
          strcpy(e_link, "->");
      }
